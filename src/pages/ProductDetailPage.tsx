    ...
    return (
      <Layout>
        <div className="container py-16 text-center">
          ...
          <Link to="/products">
            <Button variant="outline" className="mt-4 font-body">Back to Products</Button>
          </Link>
        </div>
      </Layout>
    );
      ...
      <div className="container py-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-5">
            <div>
              <p className="text-xs uppercase tracking-[0.15em] text-muted-foreground font-body mb-1">
                {product.category}
              </p>
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`h-4 w-4 ${i < Math.floor(product.rating) ? "fill-foreground text-foreground" : "text-border"}`}
                  />
                ))}
              </div>
            ...
            <div className="flex items-baseline gap-3">
              ...
              {product.originalPrice && (
                <>
                  ...
                  <span className="text-sm font-body text-success font-medium">-{discount}%</span>
                </>
              )}
            </div>
            ...
            {product.tags && product.tags.length > 0 && (
              <div className="flex gap-2 pt-2">
                {product.tags.map((tag) => (
                  <span key={tag} className="text-[10px] uppercase tracking-wider bg-accent text-accent-foreground px-2 py-1 rounded font-body">
                    {tag}
                  </span>
                ))}
              </div>
